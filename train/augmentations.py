...
class Resize(object):
    def __init__(self, min_size, max_size=1500):
        # 一般的resize都是限制短边，我们也在这里限制长边
    ...
    def __call__(self, image, boxes=None, labels=None):
        ...
        h, w, _ = image.shape
        # 首先判断把短边resize到指定大小，长边是否会崩掉
        if min(h, w) / self.min_size * max(h, w) > self.max_size:
            # 按照长边进行resize
            if h < w:
                self.size_w = self.max_size
                self.size_h = self.max_size / w * h
            else:
                self.size_h = self.max_size
                self.size_w = self.max_size / h * w
        else:
            # 短边进行resize
            if h < w:
                self.size_h = self.min_size
                self.size_w = w / h * self.size_h
            else:
                self.size_w = self.min_size
                self.size_h = h / w * self.size_w

        self.size_w = int(self.size_w)
        self.size_h = int(self.size_h)

        # if boxes is not None:
        #     new_wh = np.array([self.size_w / w, self.size_h / h], dtype=np.float32)
        #     boxes = boxes * np.tile(new_wh, 4)
        # image = cv2.resize(image.astype(np.uint8), (self.size_w, self.size_h), interpolation=cv2.INTER_CUBIC)

        # # 最简单的resize方法
        if boxes is not None:
            new_wh = np.array([self.min_size / w, self.min_size / h], dtype=np.float32)
            boxes = boxes * np.tile(new_wh, 4)
        image = cv2.resize(image.astype(np.uint8), (self.min_size, self.min_size), interpolation=cv2.INTER_CUBIC)
        return image, boxes, labels
    ...
    def __call__(self, image, boxes=None, labels=None):
        ...
        while True:
            ...
            if mode:
                ...
                for i in mode:
                    if i and i < 1:
                        mode_.append(int(i * boxes.shape[0]))
                    else:
                        mode_.append(i)
                mode = tuple(mode_)
...
class Rotate(object):
    def __init__(self, mean, deg=None):
        # self.angle_option = [0, -45, -90, 45, 90]
        # self.deg = 90
        """

        :param mean:
        :param deg: 如果传入了deg ， 就不管他
        """
        if not deg:
            self.one_side = False
            self.deg = 10
    ...
    def __call__(self, image, boxes, labels):
        ...
        if random.randint(self.rotate_prob) == 0:
            ...
            if self.one_side:
                if self.deg >= 0:
                    angle = random.randint(0, self.deg)
                else:
                    angle = random.randint(self.deg, 0)
            else:
...
class Augmentation_traininig(object):
    def __init__(self, size, mean=(104, 117, 123), deg=None):
        self.mean = mean
        ...
        self.augment = Compose([
            ConvertFromInts(),
            PhotometricDistort(),
            Rotate(self.mean, deg),
            RandomSampleCropPoly(),
            Resize(self.size_min, self.size_max),
            Normalize(),
            ToTensor(),
        ])
