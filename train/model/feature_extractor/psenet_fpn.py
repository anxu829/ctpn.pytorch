...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        c4_ = self.latlayer1(c4)
        c4_ = self.latlayer1_relu(self.latlayer1_bn(c4_))
        p4 = self._upsample_add(p5, c4_)
        p4 = self.smooth1(p4)
        p4 = self.smooth1_relu(self.smooth1_bn(p4))

        c3_ = self.latlayer2(c3)
        c3_ = self.latlayer2_relu(self.latlayer2_bn(c3_))
        p3 = self._upsample_add(p4, c3_)
        p3 = self.smooth2(p3)
        p3 = self.smooth2_relu(self.smooth2_bn(p3))

        c2_ = self.latlayer3(c2)
        c2_ = self.latlayer3_relu(self.latlayer3_bn(c2_))
        p2 = self._upsample_add(p3, c2_)
        p2 = self.smooth3(p2)
